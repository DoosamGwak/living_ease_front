...
const validation = Yup.object().shape({
  ...
  answer2: Yup.string().required(message_require),
});
...
const AIMatch = (props: Props) => {
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<answerInputs>({ resolver: yupResolver(validation) });

  const handleAnswers = (form: answerInputs) => {
    console.log(form.answer1, form.answer2);
  };

  const onClick1 = (e: SyntheticEvent) => {
    setValue("answer1", (e.target as HTMLDivElement).innerText);
  };

  const onClick2 = (e: SyntheticEvent) => {
    setValue("answer2", (e.target as HTMLDivElement).innerText);
  };
